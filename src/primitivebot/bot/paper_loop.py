...
class PaperWritingLoop:
    ...
    async def run(self, task_id: str, zip_path: str, rounds_n: int,
                  status_callback: Callable[[str], Awaitable[None]]) -> str:
        """
        ...
        for i in range(1, rounds_n + 1):
            ...
            # A. Writer Bot Phase
            await status_callback(f"📝 Round {i}/{rounds_n}: Writer Bot is drafting...")
...
"""
            ...
            await status_callback(f"🧐 Round {i}/{rounds_n}: Reviewer Bot is evaluating...")
...
"""
            # We need to make sure the reviewer can "read" the PDF.
            # In gemini-cli, it should be able to if we point it to the file or if it explores.
            # Assuming gemini-cli has tools to read PDF or we just rely on it exploring.
